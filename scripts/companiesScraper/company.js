...
async function scrapeCompany(link) {
  ...
  $('div.styles_component__DaQ39').find('a').each((i, obj) => {
  ...
  const locations = [];
  $('div.styles_component__2yJfJ').find('li').each((i, obj) => {
    if ($(obj).text() && !$(obj).text().includes('.com') && !$(obj).text().includes('Show 5 more')) {
      locations.push($(obj).text())
    }
  });


  $('div.styles_component__2yJfJ').find('dt').each((i, elem) => {
  ...
  let marketTags = [];
  $('dt.styles_tags__KR_s2').find('a').each((i, obj) => {
  ...
  $('div.styles_header__2n2Q4').each((i, elem) => {
    if ($(elem).find('h4').text()) {
      const title = $(elem).find('span.styles_byline__3ogLZ').text() || 'Founder';
      member.push({
