...
async function scrapeCompanyJobs(link) {
  const jobsPage = await request({ url: `${link}/jobs` });
  if (!jobsPage) return null;
  ...
  $('div.styles_component__1_YxE').each((i, elem) => {
    ...
    const description = $(elem)
      .find('div.styles_descriptionSnippet__yngxb')
      .text()
      .trim();
    const location = $(elem)
      .find('div.styles_truncate__dUufp')
      .text()
      .slice('•');
    let time = null;
    const link = `https://angel.co${$(elem).find('a').attr('href')}`;
    $(elem)
      .find('div.breakpoint__desktop-up')
      .each((i, elem) => {
